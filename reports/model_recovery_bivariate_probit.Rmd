...
if (params$compile){
  rstan::stan_model(file.path(devtools::package_file(), "stan", "bivariate_probit_mixed.stan"),
                               model_name = file.path(devtools::package_file(), "stan", "bivariate_probit_mixed.rds"))
}
...
d <- crossing(condition = 1:params$n_condition,
  ...
  mutate(condition_mu = map(condition, ~ cbind(params$condition_mu[.x], params$condition_mu[.x]) + mvtnorm::rmvnorm(1, mean = rep(0,2), sigma = condition_Sigma))) %>%
...
stan_data <- list(n = dim(d)[1],
                  ...
                  X = modelr::model_matrix(d, ~ factor(condition)),
                  subject = d$subject,
                  item = d$item,
                  condition = d %>% use_series(condition),
                  y = cbind(d$y1, d$y2),
                  priors = c(2, 2, 4, 2, 2, 2),
...
compiled_model <- stan_model(file.path(devtools::package_file(), "stan", "bivariate_probit_mixed.stan"))

estimated_model <- sampling(compiled_model, data = stan_data, 
                            iter = params$iter,
