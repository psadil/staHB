...
params:
  data_dir: data
  expt: CFSgonogo
  model: "bivariate_probit_ordered"
  flag: "dev"
  n_condition: 4
...
responseType = function(x) {
    ...
    if (x[1] == 0 & x[2] == 0) out = 1
    if (x[1] == 0 & x[2] == 1) out = 2
    if (x[1] == 1 & x[2] == 0) out = 3
    if (x[1] == 1 & x[2] == 1) out = 4
...
stan_data <- list(n = dim(d)[1],
                  ...
                  condition_2 = d %>% use_series(condition),
...
estimated_model_mon <- sampling(readr::read_rds(file.path(devtools::package_file(), "stan", paste0(params$model, ".rds"))), 
                                ...
                                pars = c("z_pos", "z_neg",
                                         "condition_L", "subject_L", "item_L",
                                         "item_mu_raw", "subject_mu_raw", 
                                         "Omega")
                                , include = FALSE
)
...
stan_data <- stan_data$condition_2 = d %>% use_series(condition2)
...
estimated_model_Nmon <- sampling(readr::read_rds(file.path(devtools::package_file(), "stan", paste0(params$model, ".rds"))), 
                                 ...
                                 warmup = params$warmup,
                                 chains = params$chains,
                                 pars = c("condition_L", "subject_L", "item_L",
                                         "item_mu_raw", "subject_mu_raw", 
                                         "Omega", "z")
                                , include = FALSE
)
...
d2 <- d %>% mutate(id = 1:n()) %>%
  mutate(y_type = map2(y1, y2, ~responseType(c(.x,.y)))) %>%
  dplyr::mutate(LB1 = if_else(y_type %in% c(3,4), 0, -Inf),
                LB2 = if_else(y_type %in% c(2,4), 0, -Inf),
                UB1 = if_else(y_type %in% c(3,4), Inf, 0),
                UB2 = if_else(y_type %in% c(2,4), Inf, 0)
  ) %>%
  select(id, condition, LB1:UB2)

mon <- readr::read_rds(path = file.path(devtools::package_file(), "output", "data", params$model, params$flag, "data_mon.rds")) %>%
  tidybayes::spread_samples(Mu[id,response], condition_rho[condition]) %>%
  spread(response, Mu) %>%
  rename(mu1 = `1`, mu2 = `2`) %>%
  ungroup() %>%
  inner_join(d2, by=c("id", "condition")) %>% # want a row for each trial, meaning need to drop repeated rhos
  mutate(lik = pmap_dbl(list(lb1=LB1,lb2=LB2 ,
                         ub1=UB1,ub2=UB2,
                         m1=mu1,m2=mu2, 
                         rho=condition_rho
  ),
  function(lb1,lb2,ub1,ub2, m1,m2, rho) mvtnorm::pmvnorm(lower = c(lb1,lb2),
                                                         upper = c(ub1,ub2),
                                                         mean = c(m1,m2),
                                                         corr = matrix(c(1, rho, rho, 1), nrow = 2), 
                                                         algorithm = Miwa())[1] ) 
  ) %>%
  mutate(log_lik = log(lik))

mon_log_lik_mat <- mon %>%
  select(.chain, .iteration, id, log_lik) %>%
  spread(id, log_lik) %>%
  select(-.chain) %>%
  as.matrix()
