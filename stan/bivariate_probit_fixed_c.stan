...
data {
  ...
  vector[7] priors;
  int n_orders;
  matrix[n,n_condition] X[n_orders, D]; // alt order_X
}
parameters{
...
transformed parameters {
  ...
  for(order in 1:n_orders){
    lps[order,] = theta_log[order] + biprobit_lpdf_vector(y, Mu_ordered[order], to_vector(condition_omega[condition,1,2]) );
  }
  for (i in 1:n){
