...
parameters{
  matrix<lower=0>[n_condition-1,D] condition_mu_ordered[n_orders]; // condition effects on mean of latent
...
transformed parameters {
  ...
  {
    matrix[n_condition, D] beta;

    for(order in 1:n_orders){
      beta = append_row(intercept, condition_mu_ordered[order]);

      lps[order,] = theta_log[order] +
      biprobit_lpdf_vector(y,
        append_col(X[order,1] * col(beta,1), X[order,2] * col(beta,2)),
        condition_omega[1,2] );
      }
  }
...
model {
  ...
  // priors
  for(order in 1:n_orders){
    to_vector(condition_mu_ordered[order]) ~ normal(0, priors[1]); // consider trying student_t(3,0,1);
  }
