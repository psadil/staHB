...
parameters{
  positive_ordered[n_condition-1] condition_mu_ordered[n_orders, D]; // condition effects on mean of latent
  ...
  // if put there I might need to calculate twice
    for(order in 1:n_orders){
      ...
      biprobit_lpdf_vector(y,
        append_col(X[order,1] * append_row(intercept[1], condition_mu_ordered[order,1]),
        X[order,2] * append_row(intercept[2], condition_mu_ordered[order,2])),
        condition_omega[1,2] );
      }
