...
bf1 <- bf(y1 ~ 1, mu1 ~ mo(condition), mu2 ~ mo(condition2), family = mix )
bf2 <- bf(y2 ~ 1, mu1 ~ mo(condition), mu2 ~ mo(condition2), family = mix )

f <- brms::mvbrmsformula(bf1, bf2, rescor = FALSE)

tmp <- brms::brm(formula=f, data=d, save_model = "brms.stan",
                 chains=1,
                 cores=1, iter = 1, warmup = 1)
