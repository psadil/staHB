...
params:
  ...
  flag: "single_rho-3_radius-10_radian-10"
  reps: 1
  n_condition: 3
  ...
  radius: !r seq(from = 0, to = 1, length.out = 10)
  radian: !r seq(from = pi/6, to = pi/2, length.out = 10)
  base_length: 2
  subject_scale: 0
...
post <- d_post %>%
  ...
  mutate(lpd_waic = map_dbl(waic, ~.x$elpd_waic + .x$p_waic))
...
post %>%
  ...
  ggplot(aes(x=radian_mid, y = radius_mid, fill=difference)) +
  geom_raster() +
  coord_fixed() +
