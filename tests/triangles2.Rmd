...
```{r}
d <- gen_grand_data_triangle_nr(params)
```
...
d %>%
  ...
  stat_ellipse(type = "norm", geom = "polygon", alpha = .3) +
  geom_point(alpha = .2 ) +
  coord_fixed() +
  facet_grid(condition_rho ~ radian_mid, labeller = "label_both") +
  scale_x_continuous() + # breaks  = c(-2, 0, 2), limits = c(-1.5, 1.5)
  scale_y_continuous() +
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = 0) +
  ...
  theme(strip.background = element_blank()) 
# +
#   ggsave("expt_probit_spread_radius-p1.png")
...
post <- vector(mode = "list", length = params$save_every)    

iter <- 1
for(m in 11:nrow(d_fit)){
  model <- stan_model(file.path(devtools::package_file(), "stan", paste0(params$model, ".stan")))
  post[[iter]] <- sampling(model,
