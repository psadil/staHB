...
data { 
  ...
  int Xmo_evidencey_2[N];
  // prior concentration of monotonic simplexes
  vector[Jmo_evidencey[1]] con_simo_evidencey_1;
  vector[Jmo_evidencey[2]] con_simo_evidencey_2;
...
parameters { 
  ...
  simplex[Jmo_evidencex[2]] simo_evidencex_2; 
  ...
  simplex[Jmo_evidencey[2]] simo_evidencey_2; 
...
model { 
  ...
  vector[nresp] Mu[N]; 
  for (n in 1:N) { 
    mu_evidencex[n] = mu_evidencex[n] + (bmo_evidencex[1]) * mo(simo_evidencex_1, Xmo_evidencex_1[n]) + (bmo_evidencex[2]) * mo(simo_evidencex_2, Xmo_evidencex_2[n]); 
    mu_evidencey[n] = mu_evidencey[n] + (bmo_evidencey[1]) * mo(simo_evidencey_1, Xmo_evidencey_1[n]) + (bmo_evidencey[2]) * mo(simo_evidencey_2, Xmo_evidencey_2[n]); 
    Mu[n, 1] = mu_evidencex[n]; 
    Mu[n, 2] = mu_evidencey[n]; 
  } 
  // priors including all constants 
  target += dirichlet_lpdf(simo_evidencex_1 | con_simo_evidencex_1); 
  target += dirichlet_lpdf(simo_evidencex_2 | con_simo_evidencex_2); 
  target += dirichlet_lpdf(simo_evidencey_1 | con_simo_evidencey_1); 
  target += dirichlet_lpdf(simo_evidencey_2 | con_simo_evidencey_2); 
